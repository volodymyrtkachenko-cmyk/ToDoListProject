...
function offsetForDate(date) {
    const clicked = date.getTime();
    for (let i = -52; i <= 52; i++) {
        const mon = getMonday(i).getTime();
        const sun = mon + 6 * 86400000;
        if (clicked >= mon && clicked <= sun) return i;
    }
    return 0;
}

document.addEventListener('calDateSelected', ({detail: {date}}) => {
    weekOffset = offsetForDate(date);
    renderBoard();
    renderTasks(allTasks);
});
...
function openEditModal(taskId) {
    ...
    document.querySelectorAll('.edit-category-radio').forEach(radio => {
        radio.checked = String(radio.value) === String(task.category_id);
    });

    const priMapReverse = {low: '1', med: '2', high: '3'};
    document.querySelectorAll('.edit-priority-radio').forEach(radio => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tasksDataEl) {
        try {
            allTasks = JSON.parse(tasksDataEl.textContent);
        } catch (e) {
